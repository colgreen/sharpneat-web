...
dat1 <- read.table("sinewave-samples-v2-4-1.csv", sep=",", head=TRUE)
dat2 <- read.table("sinewave-samples-v2-4-2-moff.csv", sep=",", head=TRUE)
...
p4 = ggplot(dat, aes(bestfitness, fill = ver )) + 
   ...
   scale_fill_manual(values=cbPalette, name='Version', labels=c('v 2.4.1','v 2.4.2(moff)')) +
   labs(x='fitness') + 
   ggtitle("Generative Sinewave Task") +
   theme(plot.title = element_text(size = 12))

ggsave(filename="sinewave-bestfitness-v241-v242moff-hist.png", plot=p4, height=4, width=8, units="in", dpi=100)

# Eval count.
p2 = ggplot(dat, aes(evalcount, fill = ver )) + 
   geom_histogram(alpha = 0.7, aes(y = ..density..), position = 'identity') +
   scale_fill_manual(values=cbPaletteB, name='Version', labels=c('v 2.4.1','v 2.4.2(moff)')) +
   labs(x='evaluation count') + 
   ggtitle("Generative Sinewave Task") +
   theme(plot.title = element_text(size = 12))

ggsave(filename="sinewave-evalcount-v241-v242moff-hist.png", plot=p2, height=4, width=8, units="in", dpi=100)
